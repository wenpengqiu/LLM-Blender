...
class GenerationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        name_or_path = self.tokenizer.name_or_path.lower()
        
        # --- START OF NEW LOGIC: Use Hugging Face apply_chat_template ---
        
        # 1. 构造聊天消息列表
        # 注意：Hugging Face 模板通常会自动添加 "system" 提示。
        # 我们只提供 "user" 提示。
        messages = [
            {"role": "user", "content": prompt}
        ]

        # 2. 使用 tokenizer 自带的模板
        try:
            # add_generation_prompt=True 会自动在末尾添加 'assistant' 提示符 (例如 <|start_header_id|>assistant<|end_header_id|>)
            ...
            # 如果模型没有聊天模板 (例如 flan-t5), 则回退到原始 prompt
            if any([non_conv_model in name_or_path for non_conv_model in non_conv_models]):
                final_prompt = prompt
            else:
                # 如果其他模型也失败了，打印警告并使用原始 prompt
                print(f"Warning: Failed to apply chat template for {name_or_path}. Error: {e}. Using raw prompt.")
                final_prompt = prompt

        # --- END OF NEW LOGIC ---

        if not self.template_length:
            # 估算模板长度（现在可能不太准确，但对于截断仍然有用）
        ...
        # Apply KeyError fix for 'id'
        return {
            "id": item.get('id', f'missing_id_{idx}'),
...
def get_stop_str_and_ids(tokenizer):
    ...
    """
    stop_str = None
    stop_token_ids = [] # 始终使用列表

    # --- START OF NEW LOGIC: Get stop tokens from tokenizer config ---

    # 1. 尝试从 chat_template 获取停止符
    if tokenizer.chat_template:
        try:
            # apply_chat_template 会自动处理模板并返回带停止符的 prompt
            # 我们构造一个空对话来提取末尾的停止符
            chat = [{"role": "user", "content": "test"}, {"role": "assistant", "content": "test"}]
            template = tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=False)
            
            # Llama-3 / Phi-3 等使用 <|..._id|> 格式
            if "<|eot_id|>" in template:
                stop_token_ids.append(tokenizer.convert_tokens_to_ids("<|eot_id|>"))
            if "<|end|>" in template:
                stop_token_ids.append(tokenizer.convert_tokens_to_ids("<|end|>"))
            if "<|im_end|>" in template: # Qwen 使用
                stop_token_ids.append(tokenizer.convert_tokens_to_ids("<|im_end|>"))
            if "<end_of_turn>" in template: # Gemma 使用
                stop_token_ids.append(tokenizer.convert_tokens_to_ids("<end_of_turn>"))

        except Exception as e:
            print(f"Could not parse chat_template for stop tokens: {e}")

    # 2. 始终将 EOS token 作为停止符
    ...
    print("Stop string: {}".format(stop_str)) # stop_str 可能为 None，这是正常的
...
def main(args):
    ...
    tokenizer = build_tokenizer(args.model, cache_dir=args.cache_dir)
            ...
            if os.path.exists(save_file) and not args.overwrite:
                ...
                for item in data:
                    if 'id' in item:  # <-- 添加此检查
                        ...
                        print(f"Skipping input item due to missing 'id'")
                ...
                if len(empty_ids) == 0:
                    ...
                    empty_exs = []
                    for item in data:
                        if 'id' in item and item['id'] in empty_ids:
